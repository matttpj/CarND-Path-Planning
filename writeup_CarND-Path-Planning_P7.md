...
* Reviewing **Highway Driving** project goals, project instructions and coding tips in the Q&A video
...
* Explaining in detail the code for generating paths **_[see below]_**
...
#### Submission includes all required files to run the simulator

Key files are in source directory __src/*__:
 * Connects to the simulator and initiates path planning: __main.cpp__   
 * Calls the spline library: __spline.h__
 * Helper functions for manipulation of waypoints between Map (cartesian) and Frenet coordinate systems:  __helpers.h__
 * Enables manipulation of JSON format data structures:  __json.hpp__
 * C++ template library for linear algebra including use of matrices, vectors, numerical solvers etc..:  __Eigen-3.3/*__

---
#### Compilation
Above source code compiles
